...
\begin{compactdesc}
	...
	\begin{inparaitem}[\color{red}\textbullet]
		\item $\langle \mathbf{x}, \mathbf{y+z} \rangle = \langle \mathbf{x}, \mathbf{y} \rangle + \langle \mathbf{x}, \mathbf{z} \rangle = \langle \mathbf{x+y}, \mathbf{z} \rangle $
	\end{inparaitem}
	\begin{inparaitem}[\color{red}\textbullet]
		\item $\langle \alpha \mathbf{u}, \mathbf{a} \rangle = \alpha \langle \mathbf{u}, \mathbf{a} \rangle$
	\end{inparaitem}
	\item[orthogonal:] $\mathbf{A^{-1}} = \mathbf{A^{T}}$
	\begin{inparaitem}[\color{red}\textbullet]
		\item $det(\mathbf{A}) \in +1/-1$
		\item $det(\mathbf{AA^{T}}) = 1$
	\end{inparaitem}
\end{compactdesc}
...
\begin{compactdesc}
	\item[$\partial f/\partial \mathbf{X}$:]
\begin{inparaitem}[\color{red}\textbullet]
  \item $\partial \mathbf{A} = 0$
  \item $\partial(\alpha \mathbf{X}) = \alpha \partial \mathbf{X}$
  \item $\partial(\mathbf{X}+\mathbf{Y}) = \partial(\mathbf{X}) + \partial(\mathbf{Y})$
  \item $\partial(trace(\mathbf{X})) = trace(\partial \mathbf{X})$
  \item $\partial(\mathbf{X}\mathbf{Y}) = \partial(\mathbf{X})\mathbf{Y} +\mathbf{X}\partial(\mathbf{Y})$
  \item $\partial(\mathbf{X^{-1}}) = -\mathbf{X^{-1}} (\partial\mathbf{X}) \mathbf{X^{-1}}$
  \item $\partial \mathbf{X^{T}} = (\partial \mathbf{X})^{T}$
  \end{inparaitem}
  \item[Vectors:]
  \begin{inparaitem}[\color{red}\textbullet]
  ...
  \item $\frac{\partial}{\partial \mathbf{x}}(\mathbf{s}^\top \mathbf{A} \mathbf{r}) = (\frac{\partial s}{\partial \mathbf{x}})^\top \mathbf{A} \mathbf{r} + (\frac{\partial r}{\partial \mathbf{x}})^\top \mathbf{A}^\top \mathbf{s}$
  \end{inparaitem}
  \item[scalar $\alpha$:]
  \begin{inparaitem}[\color{red}\textbullet]
  	\item $\frac{\partial}{\partial \mathbf{x}}(\mathbf{y}^\top \mathbf{A} \mathbf{x}) = \mathbf{y}^\top \mathbf{A}$
  	\item $\frac{\partial}{\partial \mathbf{y}}(\mathbf{y}^\top \mathbf{A} \mathbf{x}) = \mathbf{x}^\top \mathbf{A}^\top$
  \end{inparaitem}
  \item[Matrices:]
  \begin{inparaitem}[\color{red}\textbullet]
  \item $\frac{\partial}{\partial \mathbf{X}}(\mathbf{b}^\top \mathbf{X}^\top \mathbf{X} \mathbf{c}) = \mathbf{X} (\mathbf{b}\mathbf{c}^\top + \mathbf{c}\mathbf{b}^\top)$
  ...
  \item $\frac{\partial f}{\partial \mathbf{X}^\top} = (\frac{\partial f}{\partial \mathbf{X}})^\top$
  \end{inparaitem}
  \item[Norms:]
  \begin{inparaitem}[\color{red}\textbullet]
...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item Missing columns in $\mathbf{U}$ are basis of $\operatorname{null}(A^\top)$ and in $\mathbf{V}$ are basis of $\operatorname{null}(A)$. 
\end{inparaitem}

$\mathbf{U}$: users-to-concept affinity, $\mathbf{V}$: Movies-to-concept simil., $\mathbf{D}$: expressiveness of each concept
...
\begin{inparaenum}[\color{red} 1.]
	\item choose $K$ centroids $\mathbf{U} = [\mathbf{u}_1, \ldots, \mathbf{u}_K]$
	...
	\item goto step 2, stop if $\|\mathbf{Z} - \mathbf{Z}^\text{new}\|_0 = \|\mathbf{Z} - \mathbf{Z}^\text{new}\|^2_F = 0$.
\end{inparaenum}
...
Convolution: $F_{n,m}(\mathbf{x};\mathbf{w}) = \sigma(b + \sum_{k=-2}^2\sum_{l=-2}^{2}{w_{k,l}x_{n+k,m+l}})$.
...
Min $f(\mathbf{x})$ st $g_i(\mathbf{x}) \leq 0,\ i:1..m$ \& $h_i(\mathbf{x}) = \mathbf{a}_i^\top \mathbf{x} - b_i = 0,\ i:1..p$
\begin{compactdesc}
...
\section{Robust PCA}
\begin{compactitem}
	\item Idea: Approx. $\mathbf{X}$ with $\mathbf{L}_0 + \mathbf{S}_0$, $\mathbf{L}_0$ is low-rank, $\mathbf{S}_0$ is sparse.
