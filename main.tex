...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item $\|\mathbf{X}\|_\star = \sum_{i=1}^{\mathrm{min}(m, n)} \sigma_i$
	...
	\item $\|\mathbf{A}\|_F :=\allowbreak \sqrt{\sum_{i=1}^{M} \sum_{j=1}^{N} |\mathbf{A}_{i, j}|^2} =\allowbreak \sqrt{\operatorname{trace}(\mathbf{A}^\top \mathbf{A})} =\allowbreak \sqrt{\sum_{i=1}^{\mathrm{min}\{m, n\}} \sigma_i^2}$
...
\begin{inparadesc}
	\item[\color{red}Target:] $\mathrm{min}_{\mathbf{U}, \mathbf{Z}} J(\mathbf{U}, \mathbf{Z}) = \|\mathbf{X} - \mathbf{U} \mathbf{Z}\|_F^2 = \sum_{n=1}^N \sum_{k=1}^K z_{k,n} \|\mathbf{x}_n - \mathbf{u}_k\|_2^2$
\end{inparadesc}
...
\begin{inparaitem}[\color{red}\textbullet]
  \item $d(C,C') := \mathrm{min}_\Pi \frac{1}{2}\|\mathbf{Z}-\Pi(\mathbf{Z}')\|^2_F$ , $\Pi(\mathbf{Z}
')$ = row perm. of $\mathbf{Z}'$
  \item arbitrary sets $\mathbf{X},\mathbf{X'}$ of size $N,N'$: $r := \frac{1}{N'} \mathrm{min}_{\Pi \in \mathbb{S}_K} \{ \sum_{n=1}^{N'} \mathbb{I}_{\{\Pi(\phi(x'_n)) \neq z'_n\} } \}$
...
\begin{compactdesc}
  ...
  \item[Objective:] $H(\theta;\mathbf{N}) = \sum_{n_{ij} > 0} f(n_{ij})(\log n_{ij} - \log \exp[\langle \mathbf{x}_i, \mathbf{y}_j \rangle + b_i + d_j])^2$ with $f(n) = \mathrm{min}\{1, (\frac{n}{n_{max}})^\alpha\}$, $\alpha \in (0;1]$.
\end{compactdesc}
...
$\mathrm{min}_{\mathbf{U}, \mathbf{V}} J(\mathbf{U}, \mathbf{V}) = \frac{1}{2} \|\mathbf{X} - \mathbf{U}^\top\mathbf{V}\|_F^2$ s.t. $\forall i,j,z~u_{zi},v_{zj} \geq 0 $
...
\subsection{Unconstrained min: $\mathrm{min} f(\mathbf{x})$ with $\mathbf{x} \in \mathbb{R}^D$
}
...
\begin{inparaitem}[\color{red}\textbullet]
\item $\mathrm{min}_{\mathbf{x}} f(\mathbf{x})$ s.t. $\mathbf{A}\mathbf{x}= \mathbf{b}$
...
\item Recover optimal $\mathbf{x}^* \in \arg\mathrm{min}_{\mathbf{x}} \mathcal{L}(\mathbf{x}, \bm{\lambda}^*)$
\end{inparaitem}


\section{Sparse Coding: $\mathrm{min}_{\mathbf{z}} \|\mathbf{z}\|_0\ s.t. \|\mathbf{U}\mathbf{z}-\mathbf{x}\|_2 < \sigma $}
...
$\mathbf{U} \in \mathbb{R}^{D \times  L}$ and $L > D$. Decoding involved $\rightarrow$ add constraint $\mathbf{z}^\star \in \arg\mathrm{min}_\mathbf{z} \lVert \mathbf{z} \rVert_0$ s.t. $\mathbf{x} = \mathbf{Uz}$. NP-hard $\rightarrow$ approximate with 1-norm (convex, $\mathbf{z}^* = \arg\mathrm{min}_{\mathbf{z}} \|\mathbf{z}\|_1\ s.t. \mathbf{x} = \mathbf{U}\mathbf{z} $) or with MP.
...
\begin{compactitem}
	...
	\item $\mathrm{min}_{\mathbf{L},\mathbf{S}}\mathsf{rank}(\mathbf{L}) + \mu \lVert \mathbf{S}\rVert_0$, s. t. $\mathbf{L} + \mathbf{S} = \mathbf{X}$. As non-convex, change to $\mathrm{min}_{\mathbf{L},\mathbf{S}} \|\mathbf{L}\|_\star + \lambda \lVert\mathbf{S}\rVert_1$ (\emph{not} the same in general)
...
$ \nabla D (\bm{\lambda}) = \mathbf{A}\mathbf{x}^*-\mathbf{b}$ for $\mathbf{x}^* \in \arg\mathrm{min}_\mathbf{x} \mathcal{L}(\mathbf{x},\bm{\lambda})$

\textbf{Dual Decomposition}: $f(x)$ separable, $\mathbf{x} = \left[ \mathbf{x}_1 \dots \mathbf{x}_N \right]$

$\mathrm{min}_\mathbf{x} \left[ f(\mathbf{x}) := f_1(\mathbf{x}_1) + \dots + f_N(\mathbf{x}_N) \right]$
s.t.
...
\\$\mathbf{x}_i^{t+1} := \arg\mathrm{min}_{\mathbf{x}_i} \mathcal{L}_i(\mathbf{x}_i,\lambda^t)$; 
...
$\mathrm{min}_{\mathbf{x}_1, \mathbf{x}_2} f_1(\mathbf{x}_1) + f_2(\mathbf{x}_2)$ s. t. $\mathbf{A}_1 \mathbf{x}_1 + \mathbf{A}_2 \mathbf{x}_2 = \mathbf{b}$, $f_1, f_2$ convex
\begin{inparaitem}[\color{red}\textbullet]
